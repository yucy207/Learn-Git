...
countdata   = read.table(countfile, header = T, sep = "\t", row.names = 1, check.names = F, comment.char = "", quote = "", fill = T, stringsAsFactors = FALSE)
gene        = countdata[[1]]
names(gene) = rownames(countdata) 
...
gene1   = gene[which(names(gene) %in% rownames(count))]
result  = data.frame(circRNA_ID = rownames(DataForDEA), gene = gene1, DataForDEA, means, res, FDR = fdr ,Type = 0,check.names = F)
