...
"Usage: fpkm_density.R [options] INPUT OUTPUT
Options:
...
x  <- read.table(input, header=T, sep="\t", row.names=1, check.names=F)
#x1 <- x[, -(1:2)]
#rownames(x1) <- rownames(x)
#x <- x1
x  <- log10(x + 0.00001)
data <- stack(x)
colnames(data) <- c("fpkm", "sample")

pdf(output, width = width, height = height)
ggplot(data, aes(x =  fpkm, fill = sample, colour = sample)) +
  scale_x_continuous(limits=c(-4,4)) + 
  geom_density( alpha = 0.65) + 
  labs(y = "Density", x = "log10(fpkm)")
dev.off()
