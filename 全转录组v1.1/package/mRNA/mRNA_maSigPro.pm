package package::mRNA::mRNA_maSigPro;
...
sub run
{
	...
	if (scalar @res == 0) {
		print qq{mRNA maSigPro 基因水平时间序列分析已经运行完成!\n};
	}

	maSigPro($metadata, $base, @res, "gene");
	print qq{mRNA maSigPro 基因水平时间序列分析运行完成!\n};

	# transcript level
	my @res = res_check(qq{$result/result/transcript}, \@design);

	if (scalar @res == 0) {
		print qq{mRNA maSigPro 转录本水平时间序列分析已经运行完成!\n};
	}

	maSigPro($metadata, $base, @res,"transcript");
	print qq{mRNA maSigPro 转录本水平时间序列分析运行完成!\n};
...
{		
	...
	my $result     = qq{$metadata->{project}/mRNA/maSigPro};
	my $report     = qq{$metadata->{report}/03_mRNA_Analysis/02_Differential_Expression_Analysis/maSigPro_Analysis};
