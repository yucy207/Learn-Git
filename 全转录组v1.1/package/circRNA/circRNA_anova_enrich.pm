package package::circRNA::circRNA_anova_enrich;
...
sub run
{
	...
	my $anova    = qq{$metadata->{project}/circRNA/different_analysis/result};
	...
	if (scalar @res_groups == 0) {
		print qq{circRNA anova后续富集分析已经完成!\n};
		#return;
	}
	...
	foreach my $x (@res_groups) {
		...
		my $anova_file        = qq{$anova/$base_out/gene_ANOVA_Test_Result.xls};		
		my $enrich_out_dir    = qq{$result/result/$base_out};	
		my $enrich_report_dir = qq{$report/$base_out\_Figures};
		...
		my $extract  = qq{perl $util/extract_circRNA_anova_gene.pl $anova_file $enrich_out_dir};
	...
	print qq{circRNA anova后续富集分析运行完成!\n};
