package package::lncRNA::lncRNA_deseq;
...
sub run
{
	...
	foreach my $x (@res_groups) {
		...
		my $de_report_dir   = qq{$diff_report/$case\_vs_$control\_Differential_Expression_lncRNA_Figures};
	
		system qq{mkdir -p $de_temp_dir}            if not -d qq{$de_temp_dir};
		...
		open OUT, qq{>$de_temp_dir/gene/sample.groups} or die "Can't open $de_temp_dir/gene/sample.groups!\n";
		...
		my $trans_fmt          = qq{perl $util/deseq_trans_fmt.pl $ref_lncRNA_gtf $de_temp_dir/transcript/diff.xls > $de_temp_dir/transcript/trans.diff.xls};		
		# report pdf
	...
	my $excel     = qq{$diff_report/Differential_ Expression_lncRNA_Gene_Summary.xlsx};
	my $workbook  = Excel::Writer::XLSX->new($excel);
	my %format    = package::format::run($workbook);
	report_xlsx($workbook, \@groups, "gene", \%format, $result);
	$workbook->close();

	
	############### report:different transcript summary xlsx ############
	my $excel     = qq{$diff_report/Differential_Expression_Transcript_Summary.xlsx};
	my $workbook  = Excel::Writer::XLSX->new($excel);
	my %format    = package::format::run($workbook);	
	report_xlsx($workbook, \@groups, "transcript", \%format, $result);
	$workbook->close();

	print qq{lncRNA 差异分析运行完成!\n};
...
sub pre_check
{
	...
	die "reference lncRNA gtf file is not exists!\n" if not -e $ref_lncRNA_gtf;
...
sub report_xlsx
{
	my ($workbook, $groups, $type, $format, $result) = @_;
