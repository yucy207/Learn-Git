package package::lncRNA::lncRNA_stringTie;
...
sub run
{
	...
	my $result    = qq{$metadata->{project}/lncRNA/gene_profile}; 
	my $report    = qq{$metadata->{report}/04_lncRNA_Analysis/01_lncRNA_Expression_Profile};	

	my $util      = qq{$base->{util}};
	my $rscript   = qq{$base->{rscript_bin}};
	my $stringtie = qq{$base->{stringtie_bin}};
	my $ref_gtf   = qq{$base->{$organ}{genome_lncRNA_gtf}};
	...
	system qq{mkdir -p $report/lncRNA_Expression_Figures}  if not -d qq{$report/lncRNA_Expression_Figures};

	my @res_samples = res_check(qq{$result/result}, \@samples);
	if (scalar @res_samples == 0) {
	 	print qq{lncRNA 表达定量已经运行完成!\n};
	 	return 0;
	}  
	...
	# plot common figures
	system qq{$rscript Rscript $util/fpkm_density.R $result/result/genes.fpkm.xls $report/lncRNA_Expression_Figures/fpkm.density.pdf &> $result/log/fpkm_density.log};
	...
	system qq{$cor_plot Rscript $util/cor.r $result/result/count.norm.xls $report/lncRNA_Expression_Figures &> $result/log/cor.log};
	...
	if (exists $metadata->{'group'}) {
		...
		system qq{$pca_plot Rscript  $util/pca_plot.R $result/result/count.rlog.xls $result/result/sample.groups $report/lncRNA_Expression_Figures &> $result/log/pca.log};
		system qq{$tsne_plot Rscript $util/tsne_plot.R $result/result/count.rlog.xls $result/result/sample.groups $report/lncRNA_Expression_Figures &> $result/log/tsne.log};  

	} 

	#xlsx表格输出
	my $excel      = qq{$report/lncRNA_Expression_Summary.xlsx};
