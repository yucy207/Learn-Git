...
CNV 特殊说明	文件包含两个表格CNV Filter；CNV Original。其中CNV Original为原始CNV分析结果（去掉样本中深度低于5的记录）。CNV Filter为对原始结果进行过滤，过滤条件为：（1）CNV变异区域位于exonic (2)ControlFreq对照数据库变异频率<=0.1。(3)两个断点的同源性不能均大于10 (4)支持变异的reads数大于10 且类型为"DEL"的CN小于1.6 或者类型为"DUP"的CN大于2.4。 以上条件为并列条件
