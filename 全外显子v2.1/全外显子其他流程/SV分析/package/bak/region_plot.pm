...
sub run{
    ...
    my $outDir     = "$report/document/region_plot"; package::utils::make_dir($outDir);
...
sub plot{
    ...
    foreach my $region (sort keys %$hashSamples){ print "Plot $region ...";
        ...
        my $file       = $hashConfig  -> {'Report'}."/document/region_plot/region_text/Chr$chr-$start-$end.txt";
        ...
        foreach my $sample (@$cases)
        {
            ...
            $R -> send(qq` plot(Position,Depth,type='l',ylim=c(0,$height),main="$sample$cnv",cex.main=0.75,cex.lab=0.75,cex.axis=0.75) \n`);
