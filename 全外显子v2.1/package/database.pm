...
sub run{
    ...
    print "########## Start Database ".package::utils::get_time()." ##########\n";
    my @samples       = package::utils::get_sample($hashConfig, 'case', 'control');
    #####
    # 写日志
    #####
    package::utils::write_log_simple("$report_dir/run.log", "Database");

    # 输出文件
    my $database_snv        = "$report_dir/database.snv";
    my $analysis_all        = "$report_dir/analysis_all.txt";
    my $analysis_func       = "$report_dir/analysis.txt";
    if(package::utils::is_file_ok($database_snv, $analysis_all, $analysis_func) == 1 and package::utils::is_force_sample($hashConfig) == 0)
    {
    ...
    my $library       = "$report_dir/library/library";
    my $library_final = "$report_dir/library/library_final";
    my $vcf_final     = "$report_dir/vcf/sample.all.final.vcf.gz";
    die "Lost : $gender_log\n"    if(package::utils::is_file_ok($gender_log) == 0);
    die "Lost : $library\n"       if(package::utils::is_file_ok($library) == 0);
    die "Lost : $library_final\n" if(package::utils::is_file_ok($library_final) == 0);
    die "Lost : $vcf_final\n"     if(package::utils::is_file_ok($vcf_final) == 0);
    my @losts    = package::utils::check_vcf_sample_isOK($vcf_final, \@samples); # VCF文件样本检测，防止丢失
    die "[ERR] Lost Sample in VCF FILE: @losts\n" if(@losts > 0);

    # 开始处理
    my %hashGender       = package::gender::read_gender_file($gender_log);
