...
sub run
{
	...
	my $maarjAM_fasta = qq{$base->{maarjAM}{fasta}};
	my $maarjAM_tax = qq{$base->{maarjAM}{Taxonomy}};
	my $maarjAM_udb = qq{$base->{maarjAM}{Taxonomy_udb}};
	...
	if (-e qq{$intm_result/tmp/otu.finish}){
		...
		#return;
		print qq{$groupname OTU聚类已经运行完成!\n 是否重新运行? [y/n]};
		my $option = <STDIN>;
		$option =~ s/[\r\n]//g;
		return if $option ne "y";
		system qq{rm $intm_result/tmp/otu.finish};
		system qq{rm $intm_result/tmp/otu.log};
	}
	...
	if (uc($soft_tax) eq "M"){
		...
		#system qq{$mothur mothur "#classify.seqs(fasta=$tmpPath/otu.fasta, template=$TaxonomyFA, taxonomy=$Taxonomy, processors=10, cutoff=0)" &>> $tmpPath/otu.log};
		my $taxonomyfile = `ls $tmpPath | grep 'wang.taxonomy'`;
		chomp $taxonomyfile;    # !!!
		system qq{mv $tmpPath/$taxonomyfile $tmpPath/otu.taxonomy};
		...
		#system qq{perl $scriptdir/format_tax.pl "$tmpPath/otu.tax" "$tmpPath/otu.taxonomy"};
	...
	# stat.RA for lefse
	#system qq{$Rscript $r_script/stat2relativeAbundance.r $tmpPath/otu.tax.$dissi.stat.xls $tmpPath &>> $tmpPath/otu.log};
	...
	############################################## maarjAM ###################################################
	#选取class水平 "Archaeosporomycetes", "Glomeromycetes", "Paraglomeromycetes"
	amf($tmpPath);

	repseq("$tmpPath/otu.repseq.fasta", "$tmpPath/amf.map.ID", "$tmpPath/amf.otu.fasta");

	#MaarjAMdatabase中进行注释
	if (uc($soft_tax) eq "M"){
		system qq{$mothur mothur "#classify.seqs(fasta=$tmpPath/amf.otu.fasta, template=$maarjAM_fasta, taxonomy=$maarjAM_tax, processors=30, cutoff=0)" &>> $tmpPath/otu.log};
	}elsif (uc($soft_tax) eq "U"){
		system qq{$usearch /usearch -sintax $tmpPath/amf.otu.fasta -db $maarjAM_udb -tabbedout $tmpPath/amf.otu.tax -strand both &>> $tmpPath/otu.log};
		#system qq{perl $scriptdir/format_tax.pl "$tmpPath/amf.otu.tax" "$tmpPath/otu.taxonomy"};
		format_tax("$tmpPath/amf.otu.tax", "$tmpPath/amf.otu.taxonomy.wang.taxonomy");
	}
	#system qq{$mothur mothur "#classify.seqs(fasta=$tmpPath/amf.otu.fasta, template=$maarjAM_fasta, taxonomy=$maarjAM_tax, processors=30, cutoff=0)" &>> $tmpPath/otu.log};

	modify("$tmpPath/amf.otu_table.txt", "$tmpPath/amf.otu.taxonomy.wang.taxonomy", 0.03, 80, $tmpPath);

	repseq("$tmpPath/amf.otu.fasta", "$tmpPath/map.ID", "$tmpPath/amf.otu.repseq.fasta");

	system qq{mv $tmpPath/otu.tax.0.03.xls  $tmpPath/SILVA_otu.tax.0.03.xls};
	system qq{mv $tmpPath/otu.repseq.fasta  $tmpPath/SILVA_otu.repseq.fasta};
	
	system qq{cp $tmpPath/amf.otu.tax.0.03.xls $tmpPath/otu.tax.0.03.xls};
	system qq{cp $tmpPath/amf.otu.repseq.fasta $tmpPath/otu.repseq.fasta};
	
	system qq{cp $tmpPath/SILVA_otu.tax.0.03.xls $OTUpath/SILVA_otu.tax.0.03.xls};
	system qq{cp $tmpPath/otu.tax.0.03.xls $OTUpath/otu.tax.0.03.xls};
	system qq{cp $tmpPath/otu.repseq.fasta $OTUpath/otu.repseq.fasta};
...
sub amf{
	...
	my $tmpPath = shift;
	# my $otufile = `ls | grep 'otu.tax.0.03.xls'`;
	my $otufile = "$tmpPath/otu.tax.0.03.xls";

	my $cnt = 0;
	my $index = 0;
	my $size = 0;
	my $til  = 0;
	#my @vals = ("Archaeosporomycetes", "Glomeromycetes", "Paraglomeromycetes");
	my @vals = ("Glomeromycetes");

	open TABLE,">$tmpPath/amf.otu_table.txt" or die "Can not write file";
	open MAP,">$tmpPath/amf.map.ID" or die "Can not write file";

	open OTU, $otufile or die "Can not open $otufile, Please check it!\n";
	...
	while(<OTU>){
		...
		if ($cnt == 1){
			...
			$index++ until $arr[$index] eq "class";
			$size++ until $arr[$size] eq "OTUsize";
			$size -= 1;
			$til  = join "\t", @arr[0..$size];
			print  TABLE qq{$til\n};
			...
			if ($arr[$index] ~~ @vals){
			    ...
			    print MAP qq{$arr[0]\t$arr[0]\n};
			    my $res  = join "\t", @arr[0..$size];
				print  TABLE qq{$res\n};
