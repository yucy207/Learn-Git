...
#      Pierre Legendre, October 2007
{
	...
	if (is.complex(D.eig$values)) D.eig$values = Re(D.eig$values)
	if (is.complex(D.eig$vectors)) D.eig$vectors = Re(D.eig$vectors)
	...
	if(min.eig > -epsilon) {   # Curly 1
		...
		vectors <- sweep(as.matrix(D.eig$vectors[,1:k]), 2, sqrt(eig[1:k]), FUN="*")
		bs <- bstick.def(k)
		cum.bs <- cumsum(bs)
		...
		if((correct==2) | (correct==3)) {  # Curly 2 
			...
			if (is.complex(toto.cor$values)) toto.cor$values = Re(toto.cor$values)
			if (is.complex(toto.cor$vectors)) toto.cor$vectors = Re(toto.cor$vectors)
			...
			if(min.eig.cor > -epsilon) {   # Curly 4
				...
				vectors.cor <- sweep(as.matrix(toto.cor$vectors[,1:k2]), 2, sqrt(eig.cor[1:k2]), FUN="*")
