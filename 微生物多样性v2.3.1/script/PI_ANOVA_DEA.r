...
if(length(args) != 6){
	...
	cat(
	"Usage: 
	Rscript Picrust.ANOVA.DEA.r cogfile keggfile groupfile Threshold Type outputpath
		parameters ->
...
diff_analysis <-function(inputfile,SampleInfo,name,Type,Threshold){

	Groups = levels(SampleInfo$Group)
	inputfile = read.table(inputfile, header = T, sep = "\t", row.names = 1, check.name = F, comment.char = "", quote = "", fill = T)
	# colnames(inputfile) = gsub("-","_",colnames(inputfile))     # 全局替换
	# prepare data
	count = inputfile[,rownames(SampleInfo)]
	...
	if(length(Groups) > 2){
		...
		write.table(result,paste(name, "ANOVA_test_result.xls", sep = "_"), sep = "\t", row.names = F, quote = F)
		...
		if(nrow(SigRes) >= 1){
			...
			#layout
			width = 14 
			pdf(paste(name, "ANOVA_diff_0.05.pdf", sep = "_"),width = width ,height = height)
			layout(matrix(c(1,2), nrow = 2, byrow = T),heights = c(2,height_barplot))
			
			par(mai = c(0.1,2,0.2,2))
			plot.new() ###legend绘图列
			...
			par(las = 1, mai = c(1, 6, 0.3, 0.5),  xaxs = "i", yaxs = "i", mgp = c(3, 2, 1), lend = 2,lwd = 1.5)
...
diff_analysis(cogfile,SampleInfo,"COG",Type,Threshold)
diff_analysis(keggfile,SampleInfo,"KEGG",Type,Threshold)
