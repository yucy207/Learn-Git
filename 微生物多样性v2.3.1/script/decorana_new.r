decorana_new = function (veg, iweigh = 0, iresc = 4, ira = 0, mk = 26, short = 0, 
    before = NULL, after = NULL) 
{
    ...
    s1 <- .Fortran("eigy", x = as.double(xeig1), y = as.double(yeig1), 
        ...
        "eig")]
    if (is.na(s1$eig) | s1$eig < ZEROEIG) 
        s1$x[] <- s1$y[] <- s1$eig <- 0
    if (!ira) 
    ...
    s2 <- .Fortran("eigy", x = as.double(xeig1), y = as.double(yeig1), 
        ...
        adotj = as.double(adotj), PACKAGE = "vegan")[c("x", "y", 
        "eig")]
    if (is.na(s2$eig) | s2$eig < ZEROEIG) 
        s2$x[] <- s2$y[] <- s2$eig <- 0
    if (!ira) 
    ...
    s3 <- .Fortran("eigy", x = as.double(xeig1), y = as.double(yeig1), 
        ...
        adotj = as.double(adotj), PACKAGE = "vegan")[c("x", "y", 
        "eig")]
    if (is.na(s3$eig) | s3$eig < ZEROEIG) 
        s3$x[] <- s3$y[] <- s3$eig <- 0
    if (!ira) 
    ...
    s4 <- .Fortran("eigy", x = as.double(xeig1), y = as.double(yeig1), 
        ...
        adotj = as.double(adotj), PACKAGE = "vegan")[c("x", "y", 
        "eig")]
    if (is.na(s4$eig) | s4$eig < ZEROEIG) 
        s4$x[] <- s4$y[] <- s4$eig <- 0
    if (s1$eig > ZEROEIG) 
