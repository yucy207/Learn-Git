...
sub run
{
	...
	my $fasta       = qq{$metadata->{intm_result}/reads.clean.fasta};
	...
	# 物种注释
	if (uc($soft_tax) eq "M"){
		# system qq{$mothur mothur "#classify.seqs(fasta=$tmpPath/otu.fasta, template=$TaxonomyFA, taxonomy=$Taxonomy, processors=30, cutoff=0)" &>> $tmpPath/otu.log};
